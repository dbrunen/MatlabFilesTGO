...
fseek(fid,64,-1);
header.surname=deblank(fread(fid,22,'*char')');
header.name=deblank(fread(fid,20,'*char')');
fseek(fid,106,-1);
...
for iChan = 1 : header.Num_Chan
    ...
    header.elec(iChan).Name = deblank((fread(fid,6,'*char'))');
    header.elec(iChan).Name(isspace(header.elec(iChan).Name)) = []; % remove spaces
    header.elec(iChan).Ref = deblank((fread(fid,6,'*char'))');
